...
fn app() -> Html {
    ...
    // let contract = SimpleContract::new(addr, provider);


    // let mut f = File::open("./src/contract_abi.json").expect("file not found");
    // let mut contents = String::new();
    // f.read_to_string(&mut contents).expect("something went wrong reading the file");
    // println!("with text:\n{}", contents);
    // println!("with text aaa");
    // Contract::new(addr, )
    // let contract = SimpleContract::new(addr);
    html! {
        <h1>{ "with text aaa" } </h1>
    }
}
